...
# -------------------------
# Map setup (persistent, no flicker)
# -------------------------
m = folium.Map(
...
with col1:
    if st.button("Set Current Location"):
        st.session_state.current = [-0.7151342041984061, 37.147392541075874]  # Example
        st.success(f"✅ Current Location set: {st.session_state.current}")

with col2:
    if st.button("Set Destination"):
        st.session_state.destination = [-0.7149009222772399, 37.150788897605175]  # Example
...
if st.button("Compute Route"):
    if st.session_state.current and st.session_state.destination:
        ...
        st.warning("⚠️ Please select both current and destination.")
