...
# -------------------------
# Add markers & route dynamically
# -------------------------
m = st.session_state["map"]

if st.session_state.current:
    folium.Marker(
        st.session_state.current, popup="Current",
        icon=folium.Icon(color="green")
    ).add_to(m)

if st.session_state.destination:
    folium.Marker(
        st.session_state.destination, popup="Destination",
        icon=folium.Icon(color="red")
    ).add_to(m)

if st.session_state.route:
    folium.PolyLine(
        st.session_state.route, color="blue", weight=6, opacity=0.9
    ).add_to(m)
...
with col1:
    if st.button("Set Current Location"):
        st.session_state.current = [-0.7151342041984061, 37.147392541075874]
        st.success(f"✅ Current set: {st.session_state.current}")

with col2:
    if st.button("Set Destination"):
        st.session_state.destination = [-0.7149009222772399, 37.150788897605175]
        st.success(f"✅ Destination set: {st.session_state.destination}")
...
# -------------------------
if st.session_state.route:
    st.subheader("Route Data (JSON)")
    st.json(st.session_state.route)

    st.download_button(
        "⬇️ Download Route JSON",
